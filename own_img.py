...
train_x = train_x/255.
test_x = test_x/255.
test = np.array(train_x[1])
print ("\nthis is test shape: " + str(test.shape))
...
def predict2(w, b, X):    
    ...
    for i in range(A.shape[1]):
        ...
        pass
    return y_pred
def model(test,train_x, train_y, test_x, test_y, iterations, learning_rate):
    ...
    # predict 
    #very_scary = predict2(w,b, test)
    #print("\n\n",very_scary,"\n\n")
    idk = predict(w,b,train_x[0])
    print(idk)
    ...
    return costs
costs = model(test,train_x, train_y, test_x, test_y, iterations = 2000, learning_rate = 0.005)
